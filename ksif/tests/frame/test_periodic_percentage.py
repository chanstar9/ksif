...
class TestPeriodicPercentage(TestCase):
    ...
    def test_periodic_percentage(self):
        ...
        selected_companies = pf.periodic_percentage(min_percentage=0.25, max_percentage=0.5, factor=PER)

        # The number of selected companies times 4 is almost equal to all companies.
        self.assertAlmostEqual(len(selected_companies) / len(pf), 0.25, delta=0.001)
